...
const migrations: Migration[] = [
  ...
  {
    id: '049_project_extras',
    up(db: Database.Database) {
      const cols = db.prepare(`PRAGMA table_info(projects)`).all() as Array<{ name: string }>
      const has = (n: string) => cols.some(c => c.name === n)
      if (!has('emoji')) db.exec(`ALTER TABLE projects ADD COLUMN emoji TEXT DEFAULT '📁'`)
      if (!has('path')) db.exec(`ALTER TABLE projects ADD COLUMN path TEXT`)
      if (!has('port')) db.exec(`ALTER TABLE projects ADD COLUMN port INTEGER`)
      if (!has('tags_json')) db.exec(`ALTER TABLE projects ADD COLUMN tags_json TEXT DEFAULT '[]'`)
      if (!has('cron')) db.exec(`ALTER TABLE projects ADD COLUMN cron TEXT`)
      ...
      db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_project_files_project ON project_files(project_id, added_at);
      `)
    }
  }
]
