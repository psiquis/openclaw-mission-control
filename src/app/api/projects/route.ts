  ...
  try {
    ...
    const rows = db.prepare(`
      SELECT p.id, p.workspace_id, p.name, p.slug, p.description, p.ticket_prefix, p.ticket_counter, p.status,
             ...
             p.emoji, p.path, p.port, p.tags_json, p.cron,
             (SELECT COUNT(*) FROM tasks t WHERE t.project_id = p.id) as task_count,
             (SELECT COUNT(*) FROM tasks t WHERE t.project_id = p.id AND t.status = 'inbox') as inbox_count,
             (SELECT COUNT(*) FROM tasks t WHERE t.project_id = p.id AND t.status = 'in_progress') as in_progress_count,
             (SELECT COUNT(*) FROM tasks t WHERE t.project_id = p.id AND t.status IN ('review','quality_review')) as review_count,
    ...
    const projects = rows.map(row => ({
      ...row,
      tags: row.tags_json ? (() => { try { return JSON.parse(String(row.tags_json)) } catch { return [] } })() : [],
      tags_json: undefined,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const cron = typeof body?.cron === 'string' ? body.cron.trim() || null : null
    ...
    const result = db.prepare(`
      INSERT INTO projects (workspace_id, name, slug, description, ticket_prefix, github_repo, deadline, color, emoji, path, port, tags_json, cron, status, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', unixepoch(), unixepoch())
    `).run(workspaceId, name, slug, description || null, ticketPrefix, githubRepo, deadline, color, emoji, path, port, tags, cron)
    ...
    const row = db.prepare(`
      SELECT id, workspace_id, name, slug, description, ticket_prefix, ticket_counter, status,
             ...
             emoji, path, port, tags_json, cron, created_at, updated_at
      FROM projects
      WHERE id = ?
    `).get(Number(result.lastInsertRowid)) as Record<string, unknown>
    const project = row ? { ...row, tags: (() => { try { return JSON.parse(String(row.tags_json || '[]')) } catch { return [] } })(), tags_json: undefined } : null
