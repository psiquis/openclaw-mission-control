...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'update') {
      ...
      if (body.name && typeof body.name === 'string') job.name = body.name.trim()
      if (body.schedule && typeof body.schedule === 'string') job.schedule.expr = body.schedule.trim()
      if (typeof body.command === 'string') job.payload.message = body.command.trim()
      if (typeof body.model === 'string') job.payload.model = body.model.trim() || undefined
      if (typeof body.timezone === 'string') job.schedule.tz = body.timezone.trim() || undefined
      if (typeof body.timeout === 'number') (job.payload as any).timeoutSeconds = body.timeout
      if (typeof body.enabled === 'boolean') job.enabled = body.enabled
      if (body.delivery !== undefined) {
        if (body.delivery === 'none' || !body.delivery) {
          job.delivery = { mode: 'none' }
        } else {
          job.delivery = { mode: 'announce', channel: String(body.delivery) }
        }
      }
      job.updatedAtMs = Date.now()
