...
export async function PATCH(
  ...
  try {
    ...
    if (body?.emoji !== undefined) {
      updates.push('emoji = ?')
      paramsList.push(typeof body.emoji === 'string' ? body.emoji.trim() || '📁' : '📁')
    }
    if (body?.path !== undefined) {
      updates.push('path = ?')
      paramsList.push(typeof body.path === 'string' ? body.path.trim() || null : null)
    }
    if (body?.port !== undefined) {
      updates.push('port = ?')
      paramsList.push(typeof body.port === 'number' ? body.port : null)
    }
    if (body?.tags !== undefined) {
      updates.push('tags_json = ?')
      paramsList.push(Array.isArray(body.tags) ? JSON.stringify(body.tags) : '[]')
    }
    if (body?.cron !== undefined) {
      updates.push('cron = ?')
      paramsList.push(typeof body.cron === 'string' ? body.cron.trim() || null : null)
    }
